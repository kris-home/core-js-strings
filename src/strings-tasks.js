...
function removeFirstOccurrences(str, value) {
  const indStart = str.indexOf(value);
  const indEnd = indStart + value.length;
  if (str.substring(indStart, indEnd) === value) {
    return str.substring(0, indStart) + str.substring(indEnd, str.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indStart = str.lastIndexOf(value);
  const indEnd = indStart + value.length;
  if (str.substring(indStart, indEnd) === value) {
    return str.substring(0, indStart) + str.substring(indEnd, str.length);
  }
...
function isPalindrome(str) {
  ...
  if (sting.toLowerCase() === result.toLowerCase()) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  const result = split.map((word) => word.split('').reverse().join(''));
  return result.join(' ');
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toLowerCase() && str[i].charCodeAt() > 65) {
      result += str[i].toUpperCase();
    }
    if (str[i] === str[i].toUpperCase() && str[i].charCodeAt() > 65) {
...
function extractNameFromTemplate(value) {
  const result = value.substring(7).slice(0, -1);
  return result;
}
